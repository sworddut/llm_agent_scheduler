...
async def main():
    ...
    scheduler = Scheduler(llm_service=llm_service, tools=tools, max_concurrent_tasks=5)
    ...
    while True:
        ...
        if task_status in ["COMPLETED", "FAILED"]:
            logger.info(f"Main task {main_task.id} finished with status: {task_status}")
            break
        logger.info(f"Waiting for main task {main_task.id} to complete. Current status: {task_status}")
    ...
    final_result = scheduler.get_task_result(main_task.id)
    ...
    with open("our_system_final_report.txt", "w", encoding="utf-8") as f:
        f.write(final_result if final_result else "No result generated.")
